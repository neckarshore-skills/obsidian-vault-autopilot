...
**Status:** Phase 1 merged (PR #34, 2026-06-11) — yaml-sanity verdict eval suite (61 assertions) + CI; 3 spec gaps closed RED→GREEN. Phase 2 in-review — property-classify rule eval suite (65 assertions) + full hardening stack; 13 RED assertions closed, incl. the cooldown/birthtime safety-gate bypass (user adjustment: pulled into Phase 2) plus two newly discovered rule gaps (archive-rule slash-literal vs 099_Archive/, Layer-2 filename-in-path ambiguity). Phase 3 in-review (stacked on Phase 2) — note-quality-check destructive-path eval suite (48 assertions) + hardening; 13 RED assertions closed, incl. the ONE flagged behavior change of this plan: the Destruction Gate (Nahbereich deletes moved behind the preview/user gate, closing the E2 trust-gap finding). Merges gated on MASCHIN read-the-code PIR (#35, then Phase-3 PR). Cosmetic items #1/#2 resolved en passant, #3 deferred (user decision). Remaining: Phase 4 (launch-skill spec-contradiction fixes).
