...
- **`tag-manage` no longer destroys brand spellings** (#106). A tag the vault already spells correctly is no longer "corrected" by the capitalisation heuristic: `VibeCoding` stays `VibeCoding`, `iOS` stays `iOS`, `macos` becomes `macOS`. When two correct-looking spellings disagree (`Omnopsis` / `OMNOPSIS`) the audit makes no recommendation instead of picking one by capitalisation — a `brands` entry in your config settles it. Found in a run against a real 1,800-note vault.
- **`skill-library-sync` no longer retires skills it was never told to look for** (#90). A retirement now means the skill was actually looked for: before a note is retired, the single location that note records in its own `ort` frontmatter is checked for a `SKILL.md`. A skill still sitting there is never retired — it is reported as `unverified`, with the reason, and left exactly where it is. Found by the first run against a real vault, which planned to stamp five live skills as gone purely because no source roots were configured. An empty `source_roots` is the absence of evidence, not evidence of absence.
- **`skill-library-sync` now sees skills installed as symlinks.** A symlinked skill directory in `~/.claude/skills` was invisible to the inventory (`readdirSync` reports a symlink as "not a directory"), and a skill missing from the inventory is a skill the apply path retires. This affected the direct-symlink install mode the project documents for its own development. Dangling links, and links pointing at anything without a `SKILL.md`, are still correctly ignored.
